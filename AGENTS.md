...
- Main page `src/pages/FormBuilderPage.tsx` wires Mantine `AppShell`, preview modal (90% width, reset default values on schema change), and initial schema with default `formClass: 'FusionDocument'`. **Supports external app integration** via URL parameters (formUrl, form, formData, import) and localStorage bridge. Shows loading spinner and error alerts during form loading. See `INTEGRATION_API.md` for details.
- Form loading utility (`src/utils/formLoader.ts`) handles multiple integration methods: remote URLs, file paths, base64 data, and localStorage. Supports both JSON schemas and HTML forms with proper error handling and content-type detection.
...
- **Settings Modal** (`src/components/settings/SettingsModal.tsx`): Full-screen modal with sidebar navigation. **About section** shows version and copyright. **Integration section** (new!) shows all four external integration methods with tabbed interface, code examples, testing links, and error handling docs.
...
- **External Integration API**: Four methods to launch FusionForms from external apps (e.g., GraphCycle):
  ...
  4. Base64 data: `?formData=<base64>` - Embed data in URL
  - Comprehensive docs in `INTEGRATION_API.md`, test page at `public/integration-test.html`, GraphCycle example code in `docs/GRAPHCYCLE_INTEGRATION.ts`, quick ref in `docs/INTEGRATION_QUICK_REF.md`
...
### How to run
- **Development**: Install deps (already installed). Start dev server: `npm run dev` (runs on localhost:5173).
